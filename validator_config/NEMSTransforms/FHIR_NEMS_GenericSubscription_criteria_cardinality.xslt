...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xpath-default-namespace="http://hl7.org/fhir">
	...
	<xsl:template match="/">
		<table border="0">
			...
			<xsl:variable name="count_subscriptionRuleType" select="count(tokenize($criteria,$subscriptionRuleType))-1"/>
			<xsl:variable name="count_organization" select="count(tokenize($criteria,$organization))-1"/>
			<xsl:variable name="count_tag" select="count(tokenize($criteria,$tag))-1"/>
			...
			<xsl:call-template name="cardinality_check">
				<xsl:with-param name="count" select="$count_subscriptionRuleType"/>
				<xsl:with-param name="ref_xpath" select="$supportedevents//criterion"/>
				<xsl:with-param name="ref_attribute" select="'subscriptionRuleType'"/>
			</xsl:call-template>
			<xsl:call-template name="cardinality_check">
				<xsl:with-param name="count" select="$count_organization"/>
				<xsl:with-param name="ref_xpath" select="$supportedevents//criterion"/>
				<xsl:with-param name="ref_attribute" select="'organization'"/>
			</xsl:call-template>
			<xsl:call-template name="cardinality_check">
				<xsl:with-param name="count" select="$count_tag"/>
				<xsl:with-param name="ref_xpath" select="$supportedevents//criterion"/>
				<xsl:with-param name="ref_attribute" select="'tag'"/>
			</xsl:call-template>
		</table>
	</xsl:template>
</xsl:stylesheet>
