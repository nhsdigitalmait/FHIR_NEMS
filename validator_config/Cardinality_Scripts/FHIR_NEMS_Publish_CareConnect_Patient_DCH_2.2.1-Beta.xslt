...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xpath-default-namespace="http://hl7.org/fhir">
  ...
  <xsl:template match="/">
    <table border="0">
      ...
      <xsl:variable name="identifier_count" select="count(//*[name()='Patient']/*[name()='identifier'])" />
      <xsl:variable name="name_count" select="count(//*[name()='Patient']/*[name()='name'])" />
      <xsl:variable name="birthDate_count" select="count(//*[name()='Patient']/*[name()='birthDate'])" />
      ...
      <xsl:call-template name="cardinality_check">
        ...
        <xsl:with-param name="ref_attribute" select="'p_identifier'" />
      </xsl:call-template>    
      <xsl:call-template name="cardinality_check">
        <xsl:with-param name="count" select="$name_count" />
        <xsl:with-param name="ref_xpath" select="$supportedevents//publication/elements/element" />
        <xsl:with-param name="ref_attribute" select="'p_name'" />
      </xsl:call-template>    
      <xsl:call-template name="cardinality_check">
        <xsl:with-param name="count" select="$birthDate_count" />
        <xsl:with-param name="ref_xpath" select="$supportedevents//publication/elements/element" />
        <xsl:with-param name="ref_attribute" select="'p_birthDate'" />
      </xsl:call-template>    
    </table>
  </xsl:template>
</xsl:stylesheet>
