...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fhir="http://hl7.org/fhir" xmlns:xs="http://www.w3.org/2001/XMLSchema" xpath-default-namespace="http://hl7.org/fhir">
	...
	<xsl:variable name="rule3" select="'3: event	1..1	Fixed Value: vaccinations-1 (Vaccinations)'"/>
	<xsl:variable name="rule4" select="'4: focus	1..1	This will reference the CareConnect-Immunization-1 resource which contains information about the vaccination this event relates to.'"/>
	<xsl:template match="/">
		<table border="0">
	...
	<xsl:template name="rule3">
		...
		<xsl:variable name="fixedstring" select="'vaccinations-1'"/>
		<xsl:choose>
	...
	<xsl:template name="rule4">
		...
		<xsl:variable name="focusb" select="boolean(//Immunization/id[@value=$focus])"/>
		<xsl:choose>
			<xsl:when test="$focusb">
				<xsl:call-template name="reportPass">
					<xsl:with-param name="testString" select="$rule4"/>
					<xsl:with-param name="resultString" select="'MessageHeader focus references the Immunization resource'"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="not(focusb)">
				<xsl:call-template name="reportFail">
					<xsl:with-param name="testString" select="$rule4"/>
					<xsl:with-param name="resultString" select="'MessageHeader focus does not reference the Immunization resource'"/>
				</xsl:call-template>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
